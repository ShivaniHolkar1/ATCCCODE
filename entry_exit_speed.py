...
model = models.get('yolo_nas_m',pretrained_weights='coco').to(device) #1.yolonas small model == low accuracy but fast execution(s) 2. large model = high acuracy but low execution(l) 3 medium = (m)
path='input_files/172.18.14.7_IPC_main_20230603081533_083959 - Trim - Trim - Trim.mp4'
cap = cv2.VideoCapture(path)
frame_width = int(cap.get(3))
frame_height = int(cap.get(4))
# out = cv2.VideoWriter('video_l.mp4',cv2.VideoWriter_fourcc(*'H246'),10,(frame_width,frame_height))

totalcountup=[] #total no.of entry exit
totalcountdown=[] #total no of exit vheicle
limitdown=[100,527,999,527]
limitup = [1050,529,1700,530]
...
while (cap.isOpened()):
    ...
    if ret:
        ...
        data.write(frame) # Write a deteccted video
