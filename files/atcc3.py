...
from vehicle_database_final import Entry_vhecicle_database

device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')

model = models.get('yolo_nas_l',pretrained_weights='coco').to(device) #1.yolonas small model == low accuracy but fast execution(s) 2. large model = high acuracy but low execution(l) 3 medium = (m)

cap = cv2.VideoCapture('input_files/172.18.14.7_IPC_main_20230603081533_083959 - Trim - Trim - Trim.mp4')
...
def create_folder_according_date():    
    ...
    date_folder_path = 'E:/Python Project/Vheicle_tracking/ATCC/output_file/'+ current_date
...
def video_save():
    ...
    return detected_video_store_path,data,out
...
def draw_boxes(img, bbox,object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        if len(data_deque[id]) >= 2:
          ...
          if intersect(data_deque[id][0], data_deque[id][1], line[0], line[1]):
              ...
              if "North" in direction:#Car leaving
                ...
                if label == 'truck':
                    Entry_vhecicle_database(label,speed,cameratype,location)
                ...
                if obj_name not in object_counter1:
                    ...
                    object_counter1[obj_name] += 1
                # Entry_vhecicle_database(label,speed,cameratype,location)
                cv2.putText(img,str("Total Entering count")+':'+label,(1500,280),0,1,(0,0,0),2,lineType=cv2.LINE_AA)#Entry

            
        t_leaving=len(truckupcount)+len(carupcount)+len(busupcount)+len(bikeupcount)
        ...
        t_Entering=len(truckdowncount)+len(cardowncount)+len(busdowncount)+len(bikedowncount)
...
while (cap.isOpened()):
    ...
    if ret:
        ...
        get_data[2].write(frame) # Write a deteccted video
