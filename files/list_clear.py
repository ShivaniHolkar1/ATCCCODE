...
from new_db import Entry_vehicle_database,Exit_vehicle_database,Entry_vtype_classification,Exit_vtype_classification
...
def clear_lists(resized):

    totalcountup.clear()  # total no.of entry exit 
    totalcountdown.clear()  # total no of exit vheicle
    ...
    cardowncount.clear()
    truckdowncount.clear()

    busdowncount.clear() 
    bikedowncount.clear()
    cv2.putText(resized,str(len(carupcount)),(100,200),0,1,(0,0,0),2,lineType=cv2.LINE_AA)#Entry
        ...
        if len(data_deque[id]) >= 2:
          ...
          if intersect(data_deque[id][0], data_deque[id][1], line[0], line[1]):
              ...
              if "South" in direction: #Car entering
                ...
                Entry_vtype_classification(cardowncount,truckdowncount,busdowncount,bikedowncount)    
              ...
              if "North" in direction:#Car leaving
                ...
                cv2.putText(img,str(len(carupcount)),(100,250),0,1,(0,0,0),2,lineType=cv2.LINE_AA)#Entry

                Exit_vehicle_database(label,speed1,cameratype,location)
                Exit_vtype_classification(carupcount,truckupcount,busupcount,bikeupcount)
...
while (cap.isOpened()):
    ...
    if ret:
        ...
        cv2.putText(resized_frame,str('hhfh'),(1500,150),0,1,(0,0,0),2,lineType=cv2.LINE_AA)#Entry
        ...
        m=now.strftime("%M")
        s=now.strftime("%S")
        delta3=timedelta(hours=int(h) ,minutes=int(m),seconds=int(s))

        delta1=timedelta(hours=14,minutes=25,seconds=40)
        # print(delta3,'==',delta1)
        if delta3==delta1:
            resized_frame=clear_lists(resized_frame)
            cv2.putText(resized_frame,str(delta3)+':'+str(delta1),(100,150),0,1,(0,0,0),2,lineType=cv2.LINE_AA)#Entry
            time.sleep(1)
